...
pthread_cond_t zapisany, wyczyszczony;
...
void* producent(void* arg)
{
	...
	while(czy <= (KONIEC * ILOSC))
	{
		pthread_mutex_lock(&muteks);
			while(i == ILOSC)
				pthread_cond_wait(&wyczyszczony, &muteks);
			push_front(wartosc);
			printf("producent wyprodukował: %d\n", wartosc++);
			if(i == 1)
				pthread_cond_signal(&zapisany);
			usleep(70000);
			czy++;
...
void* konsument(void* arg)
{
	...
	while(czy <= (KONIEC * ILOSC))
	{
		pthread_mutex_lock(&muteks);
			while(i == 0)
				pthread_cond_wait(&zapisany, &muteks);
			wynik = pop_front();
			printf("-----------------------konsument skonsumował : %d\n", wynik);
			if(i == ILOSC - 1)
				pthread_cond_signal(&wyczyszczony);
			usleep(70000);
			czy++;
		pthread_mutex_unlock(&muteks);
	}
...
int main()
{
	...
	pthread_mutex_init(&muteks, NULL);
	{
		...
		pthread_cond_init(&zapisany,  NULL);
